...
class Detect(nn.Module):
    ...
    class Detect(nn.Module):
        ...
        def _make_grid(self, nx=20, ny=20, i=0): #生成了一个网格和一个锚框网格
...
class Model(nn.Module):
    def __init__(self, cfg='yolov3.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch])  # model, savelist #构建yolo模型 把网络分块实例化放进列表里里
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        return self._forward_once(input, profile, visualize)  # single-scale inference, train #return x 特征图
    ...
    def _forward_once(self, x, profile=False, visualize=False):#执行这里【1,256,16,16】
        y, dt = [], []  # outputs
        for m in self.model:   #因为self.model = [BasicBlock,BasicBlock_1,BasicBlock_2,Conv_A,...]

            if m.f != -1:  # if not from previous layer # 判断将哪一层作为下一层的输入功能就是把两个特征图合并到一起
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
            if profile:
                self._profile_one_layer(m, x, dt)
            x = m(x)  # run #一层一层执行 Detect层 输入 x=[y[6],y[10]]
...
def parse_model(d, ch):  # model_dict, input_channels(3) #ch=[3]
    ...
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors 几个锚点
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5) 输出向量的的长度（特征图的维度）
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m is Detect: #执行这个
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f) #[nc,anchors]
        elif m is Contract:
            c2 = ch[f] * args[0] ** 2
        elif m is Expand:
            c2 = ch[f] // args[0] ** 2
        else:   # m = sample，Concat
            c2 = ch[f]

        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module #实际上是对各个模块进行实例化 n没有大于1的
        ...
        if i == 0:
            ch = []
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save) # *号是表示把列表中的内容分别输入进去
