...
class ActFun(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        temp = abs(input - thresh) < lens #2len/2
        temp = temp / (2 * lens)#？
        return grad_input * temp.float()

act_fun = ActFun.apply

class mem_update(nn.Module): #LIF模型
...
class batch_norm_2d(nn.Module):  #这应该就是TDBN正则化操作
...
class batch_norm_2d1(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1):
        ...
        self.bn = BatchNorm3d2(num_features) #对应公式W*X+b W已经置为阈值，b为0
...
class BatchNorm3d1(torch.nn.BatchNorm3d):#torch.nn.BatchNorm3d就是正则化公式的形式，下面把参数改变之后就符合了文中的公式
    def reset_parameters(self):
        self.reset_running_stats()
        if self.affine:
            nn.init.constant_(self.weight, thresh) #thresh写在这里 \lamda为thresh 应该是乘上阈值
            nn.init.zeros_(self.bias) #\beta为0
...
class BatchNorm3d2(torch.nn.BatchNorm3d):
    def reset_parameters(self):
        ...
        if self.affine:
            ...
            nn.init.constant_(self.weight, 0.2*thresh)   #这一步是把w置为阈值
...
class BasicBlock_1(nn.Module):#
    def __init__(self, in_channels, out_channels, stride=1,e=0.5):
        ...
        c_=1024
        self.residual_function = nn.Sequential(   #follow MS—ResNet的部分
        ...
        if stride != 1 or in_channels != out_channels: #对应改进的网络结构
            self.shortcut = nn.Sequential(
...
class Concat_res2(nn.Module):#合并特征
